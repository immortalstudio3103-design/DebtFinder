...
const snakeControlButtons = document.querySelectorAll(".snake-control-btn");
...
function initSnakeGame() {
  if (!snakeBoard || !window.SnakeLogic) return;

  const GRID_SIZE = 16;
  const TICK_MS = 140;
  let gameState = window.SnakeLogic.createInitialGameState(GRID_SIZE);
  const cells = [];

  snakeBoard.style.gridTemplateColumns = `repeat(${GRID_SIZE}, 1fr)`;
  ...
  function render() {
    ...
    gameState.snake.forEach((part, index) => {
      if (part.x < 0 || part.y < 0 || part.x >= GRID_SIZE || part.y >= GRID_SIZE) return;
      const cell = cells[getCellIndex(part.x, part.y)];
      if (!cell) return;
      cell.classList.add(index === 0 ? "snake-head" : "snake-body");
    });

    snakeScore.textContent = `Score: ${gameState.score}`;
    if (gameState.isOver) {
      snakeStatus.textContent = "Game over. Press Restart.";
      snakePauseBtn.textContent = "Pause";
    } else if (gameState.isPaused) {
      snakeStatus.textContent = gameState.started ? "Paused." : "Press any direction key to start.";
      snakePauseBtn.textContent = "Resume";
    } else {
  ...
  function applyDirection(direction) {
    gameState = window.SnakeLogic.setDirection(gameState, direction);
  ...
  function restart() {
    gameState = window.SnakeLogic.createInitialGameState(GRID_SIZE);
    render();
  }
  ...
  snakeRestartBtn.addEventListener("click", restart);
  ...
  window.setInterval(() => {
    const nextState = window.SnakeLogic.stepGame(gameState);
